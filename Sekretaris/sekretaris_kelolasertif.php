...
$where_clause = "WHERE w.status = 'finished'"; 
...
if (!empty($filter_periode)) {
    ...
    if (count($parts) == 2) {
        ...
        $tahun = $parts[1];
        if ($semester == 'Genap') {
            $start = "$tahun-01-01"; $end = "$tahun-06-30";
        } else {
            $start = "$tahun-07-01"; $end = "$tahun-12-31";
        }
        $where_clause .= " AND w.tanggal_direncanakan BETWEEN '$start' AND '$end'";
    }
}
...
// 2. QUERY UTAMA
// ---------------------------------------------------------
$query_webinars = "SELECT w.*, GROUP_CONCAT(n.nama SEPARATOR ', ') AS daftar_narasumber
                   FROM webinars w
...
$result_webinars = mysqli_query($koneksi, $query_webinars);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><title>Kelola Sertifikat</title>
    ...
    <div class="container">
        ...
        <main class="main-content">
            ...
            <form method="GET" action="" class="filter-container">
                ...
                <?php if(!empty($filter_periode)): ?><a href="sekretaris_kelolasertif.php" class="btn-reset"><i class="fas fa-times"></i> Reset</a><?php endif; ?>
            </form>

            <div class="content-card">
                <table>
                    <thead><tr><th>Narasumber</th><th>Tanggal</th><th>Topik Webinar</th><th>Aksi</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="/projek-ifws/Sekretaris/sekretaris_detailsertif.php?id_webinar=<?= $webinar['id'] ?>" class="btn btn-upload">Kelola</a>
                            </td>
                        </tr>
                        <?php endwhile; else: ?>
