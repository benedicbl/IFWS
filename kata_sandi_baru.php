...
if (isset($_GET['token']) && isset($_GET['email'])) {
    ...
    // Cek apakah token cocok & belum expired
    $query = "SELECT id FROM peserta WHERE email = '$email' AND reset_token = '$token' AND reset_expires > NOW()";
    $result = mysqli_query($koneksi, $query);

    if (mysqli_num_rows($result) == 0) {
        // Token salah atau expired
        header("Location: login.php?error=Link tidak valid atau sudah kadaluarsa.");
        exit();
    }
} else {
...
<head>
    <meta charset="UTF-8">
    <title>Kata Sandi Baru - IFWS</title>
...
<body>
    <div class="container">
        <div class="left-panel">
            ...
            <p class="subtitle">Silahkan masukkan kata sandi yang terbaru!</p>

            <?php if (isset($_GET['error'])): ?>
                <div class="alert"><?= htmlspecialchars($_GET['error']) ?></div>
            <?php endif; ?>

            <form action="api/process_new_password.php" method="POST">
