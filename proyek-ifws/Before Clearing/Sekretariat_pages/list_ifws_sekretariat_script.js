document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- FAKE DATA (Diperbarui dengan status 'isRecapped') ---
    const webinarData = {
        '2025-Ganjil': [
    ...
    // --- FUNGSI UTAMA (dengan logika tombol kondisional) ---
    function displayWebinars(tahun) {
        ...
        if (data.length > 0) {
            data.forEach(webinar => {
                ...
                if (webinar.isRecapped) {
                    // DIUBAH: Mengarah ke lihat_rekap.html
                    actionButtonHtml = `<a href="lihat_rekap.html?id=${webinar.id}" class="btn-lihat">Lihat</a>`;
                } else {
                    // DIUBAH: Mengarah ke rekap_peserta.html (halaman import)
                    actionButtonHtml = `<a href="rekap_peserta.html?id=${webinar.id}" class="btn-rekap">Rekap</a>`;
                }
